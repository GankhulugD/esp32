...
function getDayLabel(unixSecs: number) {
  return DAY_LABELS[new Date(unixSecs * 1000).getDay()];
}
...
function formatDate(unixSecs: number) {
  ...
  if (d.toDateString() === today.toDateString()) return `Today, ${formatTime(unixSecs)}`;
  if (d.toDateString() === yesterday.toDateString()) return `Yesterday, ${formatTime(unixSecs)}`;
  return d.toLocaleDateString("en-US", { month: "short", day: "numeric" }) + `, ${formatTime(unixSecs)}`;
}
    ...
    <div className="px-4 pt-6 space-y-5">
      ...
      <div className="bg-blue-50 rounded-2xl p-5">
        ...
        <div className="grid grid-cols-2 gap-4 mt-4">
          <div>
            <p className="text-[10px] text-gray-400 uppercase tracking-wider">TOTAL FOOD CONSUMED</p>
            <p className="text-2xl font-bold text-blue-600 mt-1">
              {loading ? "—" : `${stats?.totalCups ?? 0} cups`}
            </p>
          </div>
          <div>
            <p className="text-[10px] text-gray-400 uppercase tracking-wider">AVERAGE FEEDING TIME</p>
            <p className="text-2xl font-bold text-blue-600 mt-1">
      ...
      <div className="bg-blue-600 rounded-2xl p-5 text-white">
        <div className="flex items-center justify-between mb-2">
          <div className="flex items-center gap-2">
            ...
            <span className="text-sm font-semibold">Tank Level</span>
          </div>
          <span className="text-[10px] bg-green-400/30 text-green-300 px-2 py-0.5 rounded-full font-semibold">
            ● ONLINE
          </span>
        </div>
        <p className="text-xs text-blue-200 mb-3">Last filled 2 days ago</p>
      ...
      <div className="bg-white rounded-2xl p-5 shadow-sm border border-gray-100">
        <div className="flex items-center justify-between mb-1">
          <p className="text-sm font-semibold text-gray-800">Consumption Trends</p>
          <div className="flex items-center gap-1">
            <span className="w-2 h-2 rounded-full bg-blue-500" />
            <span className="text-[10px] text-gray-400">Actual</span>
          </div>
        </div>
        <p className="text-[10px] text-gray-400 mb-4">Daily food usage in cups</p>
        {loading ? (
          ...
          <ResponsiveContainer width="100%" height={130}>
            <BarChart data={chartData} margin={{ top: 0, right: 0, left: -24, bottom: 0 }}>
              ...
              <Tooltip
                contentStyle={{ background: "#fff", border: "none", borderRadius: 10, boxShadow: "0 4px 20px rgba(0,0,0,0.08)", fontSize: 11 }}
                formatter={(v) => [`${v} cups`, "Consumed"]}
              />
      ...
      <div>
        <div className="flex items-center justify-between mb-3">
          <p className="text-sm font-semibold text-gray-800">Feeding History</p>
            ...
            recentHistory.map((item: HistoryItem) => (
              ...
              >
                <div className={`w-9 h-9 rounded-full flex items-center justify-center ${
                  item.triggeredBy === "manual" ? "bg-orange-100" : "bg-blue-50"
                }`}>
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-xs font-semibold text-gray-700">
                    {item.triggeredBy === "manual" ? "Manual Snack" : "Scheduled Meal"}
                  </p>
                  <p className="text-[10px] text-gray-400">{formatDate(item.createdAt)}</p>
                </div>
                <span className="text-xs font-bold text-gray-700">{item.portionCups} cups</span>
